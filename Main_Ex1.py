...
class ModelSetup(pp.ContactMechanicsBiot, pp.ConformingFracturePropagation):
    def __init__(self, params):
        ...
        self.time_step = 1*60*60/4
        self.time_step_fix = 1*60*60/4
    ...
    def set_rock_and_fluid(self):
        ...
        self.INJECTION = 5e-9 #(m^2/s)
        self.phase = 1
        #( m2/s)
        ## C2
        self.Sy = 20E6  # pressure (Pa)
        self.Sx = 9E6  # pressure (Pa)
        
        ## C1
        # self.Sy = 9E6  # pressure (Pa)
    ...
    def set_permeability_from_aperture(self):
        ...
        for g, d in gb:
            ...
            K = pp.SecondOrderTensor(1*kxx, 1*kxx)
    ...
    #         print('injection')
    #         dis = np.sqrt((g.cell_centers[0,:] - self.inject_point[0])**2 + (g.cell_centers[1,:] - self.inject_point[1])**2)
    #         inject = np.where(dis <= tol)[0]
    #         values[inject] = self.INJECTION*self.time_step  # self.INJECTION m^2/s
    #     return values
...
defor = np.copy(p)
defor[:,0] = defor[:,0] + disp_nodes[:,0]*1e2
defor[:,1] = defor[:,1] + disp_nodes[:,1]*1e2
analysis.trisurf( defor, t, fn = None, point = None, value = setup.stored_pres[kk]*1e-6)
...
# a_file = open("Ex1_C1.pkl", "wb")
# pickle.dump(solution, a_file)
# a_file.close()

# import pickle
# a_file = open("Ex1_C1.pkl", "rb")
