...
class ModelSetup(pp.ContactMechanicsBiot, pp.ConformingFracturePropagation):
    def __init__(self, params):
        ...
        self.length_scale = 1
        self.micro_size = 0.12*self.length_scale #0.23 0.33 0.5
        ...
        self.mesh_args = { "mesh_size_frac": 2*self.mesh_size/2, "mesh_size_min": 1 * self.mesh_size, "mesh_size_bound": 12* self.mesh_size} 
        ...
        # fracture2 = np.array([[0.85, 0.95],
        #                       [1.15, 1.05]])*self.length_scale
        ...
        lenfra = 0.4
        phi = np.pi/4
        fracture2 = np.array([[xcen - lenfra/2*np.cos(phi), ycen - lenfra/2*np.sin(phi)],
                              [xcen + lenfra/2*np.cos(phi), ycen + lenfra/2*np.sin(phi)]]) 


        
        self.inject_point = np.array([1.0, 1.1])
    ...
    def set_rock_and_fluid(self):
        ...
        # C1: 1e-7, C2: 5e-8, C3: 5e-7
        self.INJECTION = 1e-7 #c1: 5e-8, c2: 5e-9, c3: 1e-7
        self.phase = 1
        #( m2/s)
        self.Sy = 10E6  # pressure (Pa)
        self.Sx = 20E6  # pressure (Pa)
    ...
    def set_permeability_from_aperture(self):
        ...
        for g, d in gb:
            ...
            if g.dim == 1:
                ...
                kyy = self.PERMEABILITY/self.VISCOSITY * np.ones(g.num_cells)
                
                bou1 = g.cell_centers[0,:] - (g.cell_centers[1,:] - 1)**2 - 1.2
                bou2 = g.cell_centers[0,:] - (g.cell_centers[1,:] - 1)**2 - 1.4
                bou0 = np.intersect1d(np.where(bou1 > 0)[0], np.where(bou2 < 0)[0])
                
                kyy[bou0] = 1e2*kyy[bou0] # C1 x 1e3, c2 x 1e6
                K = pp.SecondOrderTensor(kxx, kyy)
            else:
                kxx = self.PERMEABILITY/self.VISCOSITY * np.ones(g.num_cells)
                K = pp.SecondOrderTensor(kxx)
    ...
    def _bc_values_mechanics(self, g):
        ...
        values[1, north] = -self.Sy*g.face_areas[north]
        # values[1, south] = self.Sh*g.face_areas[south]
        values[0, east] = -self.Sx*g.face_areas[east]
    ...
    def propagation_small_scale(self, fracture_small, newfrac, tips, disp_cells, pres_cells, QPE):    
        ...
        pos_pro = []
        if np.max(keq) >= self.material['KIC']:
            pos_pro = np.where(keq*1.1 >= 0*self.material['KIC'])[0]
            # pos_pro = np.array([2,3])
            # pos_pro = np.where(np.abs(keq) >= 0)[0]
        
        if len(pos_pro) > 0:
            # ladv[pos_pro] = self.fra_increment*( G[pos_pro]/np.max(G[pos_pro]) )**0.35
            ladv[pos_pro] = self.fra_increment
...
analysis.trisurf( p + disp_nodes*1e2, t, fn = None, point = None, value = setup.stored_pres[kk]*1e-6, vmin = 6, vmax = 15)  #vmin = 5, vmax = 12
...
# solution = dict([('times', time), ('nodes', p), ('cells', t), ('displacements', disp), ('pressures', pre), 
#                   ('fractures', frac), ('pressures_1d', pre1) , ('traction', trac), ('aperture', aper)])        
# a_file = open("Ex2_C1.pkl", "wb") # C1 X1000
# pickle.dump(solution, a_file)
# a_file.close()

# import pickle
# a_file = open("Ex1_C2.pkl", "rb")
# solu = pickle.load(a_file)

# kk = 32
# p = solu['nodes'][kk]
# t = solu['cells'][kk]
# disp_nodes =  analysis.NN_recovery( solu['displacements'][kk], p, t)
# pres_nodes =  analysis.NN_recovery( solu['pressures'][kk], p, t)
# analysis.trisurf( p + disp_nodes*1e2, t, fn = None, point = None, value = solu['pressures'][kk]*1e-6, vmin = 6, vmax = 15)  #vmin = 5, vmax = 12
