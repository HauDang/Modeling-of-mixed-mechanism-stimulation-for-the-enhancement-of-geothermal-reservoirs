...
class ModelSetup(pp.ContactMechanicsBiot, pp.ConformingFracturePropagation):
    def __init__(self, params):
        ...
        self.fracture = np.array([fracture1, fracture2, fracture4])
    ...
    def set_rock_and_fluid(self):
        ...
        self.INJECTION = 5e-9 # 1E-7, 5E-8, 1E-8, 5E-9 1e-9
        self.phase = 1
        #( m2/s)
        self.SH = 20E6  # pressure (Pa)
        self.Sh = 10E6  # pressure (Pa)
    ...
    def after_newton_convergence(self, solution, errors, iteration_counter):
        ...
        if CREATE_GRID_AGAIN:
            print('connect')
            self.time_step = 1*60*60*0.5
            self.time_step_fix = 1*60*60*0.5
            # self.previous_pro = True
            self.phase = 2
...
# a_file = open("Ex3_C4.pkl", "wb")
# pickle.dump(solution, a_file)
# a_file.close()


# import pickle
# a_file = open("Ex3.pkl", "rb")
...
for i in range(len(xx)):
    fraci = xx[i]
    for j in range(fraci.shape[0] - 1):
        dis = dis + np.sqrt(np.sum((fraci[j+1,:] - fraci[j,:])**2))
